...
Mirrors run_tabular_baselines.py / run_stgcn.py: loads each (percentile, fold)
once, fits a GRU on per-node sequences, evaluates with the same metrics as the
other baselines, and writes per-fold JSON + a cumulative metrics_summary.csv.
"""
...
def run_one(
    ...
    return [
        result_row(model_name, percentile, fold, "val", val_metrics, fold_data.train_summary, model_info),
        result_row(model_name, percentile, fold, "test", test_metrics, fold_data.train_summary, model_info),
    ]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Target z[t] is predicted from inputs up to t-pred_horizon (1 = forecast, matches STGCN).",
    )
