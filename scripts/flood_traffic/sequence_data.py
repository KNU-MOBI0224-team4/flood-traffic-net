...
def build_sequence_rows(
    ...
    for t in timestamps:
        ...
        for n in range(node_count):
            ...
            if not static_ok[n]:
                continue
            seq_x = X_dynamic[t - seq_len + 1 : t + 1, n, :]
            seq_mask = input_mask[t - seq_len + 1 : t + 1, n, :]
            if not seq_mask.all():
                continue
            static_feat = static[n]
            static_seq = np.repeat(
                static_feat[None, :],
                seq_len,
                axis=0,
            )
            ...
            y_list.append(np.uint8(z[t, n]))
            time_list.append(np.int32(t))
            node_list.append(np.int32(n))

    X_out = np.asarray(X_list, dtype=np.float32)
    y_out = np.asarray(y_list, dtype=np.uint8)

    return GRUSplit(
        X=X_out,
        y=y_out,
        global_t=np.asarray(time_list, dtype=np.int32),
        node_idx=np.asarray(node_list, dtype=np.int32),
    )
...
def load_fold_data(
    ...
    X_dynamic = np.load(data_dir / "features/X_dynamic.npz", mmap_mode="r")
    ...
    train_split = build_sequence_rows(X_dynamic, input_mask, z, z_mask, static, selected_train_ts)
    val_split = build_sequence_rows(X_dynamic, input_mask, z, z_mask, static, val_ts)
    test_split = build_sequence_rows(X_dynamic, input_mask, z, z_mask, static, test_ts)
